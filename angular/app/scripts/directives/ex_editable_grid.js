...
/**
 * @ngdoc directive
 * @name angularApp.directive:exEditableGrid
 * @description
 * # exEditableGrid
 */
angular.module('angularApp')
  .directive('exEditableGrid', function ($compile, $sce) {
    ...
    function updateView() {
      var html = '<div class="cls-ex-editable-grid">';
      ...
      if(!scope.typeFixed) {
        ...
        html += '  <a href="" ng-click="newItem()" class="btn btn-primary btn-xs" ng-hide="viewOnly">';
      ...
      angular.forEach(scope.columns, function(c) {
        ...
        if(c.fixed) {
          ...
          if(c['lookup']) {
            _body += '<td ' + c.attrs + ' ng-hide="editorEnabled($index)">{{ (lookup_' + c['lookup'] + ' | byValue:item.' + c.name + ').name' + getFilter(c) + ' }}</td>';
          } else {
      ...
      _header += '<th class="grid-action-2" ng-hide="viewOnly"></th>';

      // Action buttons
      _body   += '<td ng-hide="viewOnly">' + getButtons() + '</td>';
