...
angular.module('app').directive('bsSwitch', ['$timeout', function ($timeout) {
  return {
    template:
      '<div data-on-label="{{switchOnLabel}}" data-off-label="{{switchOffLabel}}" data-text-label="{{switchLabel}}" data-icon-label="{{switchIcon}}" data-animated="{{switchAnimate}}" ng-class="{{getSizeClass()}}"><input ng-model="ngModel"/></div>',
    restrict: 'EA',
    replace: true,
    transclude: true,
    scope: {
      ...
      return function link(scope, element, attrs) {
        ...
        var listenToModel = function() {
          ...
          scope.$watch('switchIcon', function(newValue) {
            if (newValue) {
              element.find('label').html('<i class="glyphicon glyphicon-'+ newValue +'"></i>');
            } else {
